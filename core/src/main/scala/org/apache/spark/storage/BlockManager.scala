...
private[spark] class BlockManager(
  ...
  private def doPut[T](
    ...
    /* 实例化一个要存储的BlockInfo, 如果blockId对应的BlockInfo不存在，那么进行实例化，并返回；如果已经存在，则返回None*/
    val putBlockInfo = {
      val newInfo = new BlockInfo(level, classTag, tellMaster)
      // 可能会有多个线程对blockId进行写操作,如果blockInfoManager的锁对象被别的线程占用了，说明可能有别的线程在调用这个方法
      // 所以这里可能会阻塞，直到独占锁的对象释放该锁
    ...
    val result: Option[T] = try {
      ...
      if (res.isEmpty) {
        ...
        // 如果block成功存储，就进行锁的释放或者降级
        if (keepReadLock) {
          // 如果保持共享读锁，则把独占锁降级为共享读锁
          blockInfoManager.downgradeLock(blockId)
        } else {
          //否则释放blockId对应对象的锁
  ...
  private def doPutIterator[T](
    ...
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
      ...
      var size = 0L
      if (level.useMemory) {
        // 如果存储级别使用内存，优先放入内存, 如果内存hold不住，就溢出到磁盘
        // Put it in memory first, even if it also has useDisk set to true;
        // We will drop it to disk later if the memory store can't hold it.
        if (level.deserialized) {
          memoryStore.putIteratorAsValues(blockId, iterator(), classTag) match {
            // 内存足够存放整个block，就直接计算，否则通过Iterator来进行计算
            case Right(s) =>
              size = s
            case Left(iter) =>
