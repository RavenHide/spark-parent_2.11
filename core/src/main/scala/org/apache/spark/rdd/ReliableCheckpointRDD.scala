...
private[spark] object ReliableCheckpointRDD extends Logging {
  ...
  def writeRDDToCheckpointDirectory[T: ClassTag](
    ...
    /* checkpoint的主要流程有
        1.获取要进行checkpoint的rdd的sparkContext的hadoopConfig，然后实例化出hdfs的客户端FileSystem对象(DistributedFileSystem)
        2.把hadoop的config文件放入broadcast里面
        3.sparkContext进行一个runJob，在excetor里面执行writePartitionToCheckpointFile方法
          3.1. 在excetor里面执行writePartitionToCheckpointFile方法的主要逻辑就是把rdd的结果存入到hdfs里面
        4.然后再driver端，直接调用writePartitionerToCheckpointDir，把rdd的partitioner给存到hdfs里面
        5.返回一个CheckPointRDD的子类
     */
