...
abstract class RDD[T: ClassTag](
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    // ShuffleMapTask里面的runTask会调用SparkEvn的shuffleManager成员变量里的writer来调用rdd这个iterator方法
    // 也是Task任务执行的开始
    if (storageLevel != StorageLevel.NONE) {
      // 判断存储级别，如果不是None，则检查是否有缓存，有缓存直接从缓存里面拿，没有则进行计算
      // getOrCompute主要是调用BlockManager.getOrUpdate方法来执行
      getOrCompute(split, context)
    } else {
      // 没有缓存则从checkpoint里面读取或者重新计算
  ...
  private[spark] def computeOrReadCheckpoint(split: Partition, context: TaskContext): Iterator[T] =
  {
    ...
    if (isCheckpointedAndMaterialized) {
      // 当一个RDD进行checkpoint时，会丢弃他的dependency，所以这里的firstParent指向的是最新进行checkpoint的RDD
  ...
  private[spark] def getOrCompute(partition: Partition, context: TaskContext): Iterator[T] = {
    ...
    SparkEnv.get.blockManager.getOrElseUpdate(blockId, storageLevel, elementClassTag, () => {
      ...
      case Right(iter) =>
        new InterruptibleIterator(context, iter.asInstanceOf[Iterator[T]]) // 数据过大，需要用iterator来处理
    }
  }
